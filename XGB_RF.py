...
def user_input():
    ...
    HH_imcorr2 = st.sidebar.slider('HH_imcorr2', 0.9910,0.9999,0.9920)
    HV = st.sidebar.slider('HV', -18, -7 , -12)
    HV_diss = st.sidebar.slider('HV_diss', 60,700,100)
    HV_inertia = st.sidebar.slider('HV_inertia', 22,700,100)
    HV_var = st.sidebar.slider('HV_var', 23,200,150)
    NDMI = st.sidebar.slider('NDMI', -1.00,1.00,0.01)
    NDVI = st.sidebar.slider('NDVI', -1.00,1.00,0.01)
